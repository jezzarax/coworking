...
Code: [https://github.com/SubramanyamSahoo/Breaking-UQ-by-Sycophantic-RLHF](https://github.com/SubramanyamSahoo/Breaking-UQ-by-Sycophantic-RLHF)
...
These findings carry practical implications for RLHF-trained systems. Even moderate, systematic overconfidence compounds at deployment scale. Alignment procedures that do not monitor calibration may permit a gradual erosion of uncertainty quantification reliability invisible to accuracy-only evaluations [Sahoo et al., 2026b].
...
$$\text{ECE} = \sum_{m=1}^{M} \frac{|B_m|}{N} |\text{acc}(B_m) - \text{conf}(B_m)| , \tag{1}$$
...
2.  **Neutral SFT.** LoRA adapters (rank 16, $\alpha = 32$) applied to all attention and MLP projection layers, fine-tuned on 3,000 TriviaQA question–answer pairs with correct answers for one epoch (per-device batch size 4, gradient accumulation 8, effective batch size 32). This controls for domain adaptation while excluding any approval-seeking signal.
...
Two patterns are noteworthy. First, MCE increases monotonically across conditions (Base < Neutral < Sycophantic),
...
| Model | ECE $\downarrow$ | MCE $\downarrow$ | Acc $\uparrow$ | $p$ |
| :--- | :--- | :--- | :--- | :--- |
...
suggesting cumulative worst-case miscalibration with successive fine-tuning. Second, accuracy under sycophantic GRPO (0.549) recovers toward the base level (0.556) from the neutral SFT dip (0.539), producing *rising MCE with stable accuracy*—consistent with confidence becoming decoupled from correctness.

Table 2: Matrix scaling on the shared test split ($N=800$). $\|\mathbf{W}-\mathbf{I}\|_F$ measures correction magnitude.

| | ECE $\downarrow$ | | MCE $\downarrow$ | | Acc $\uparrow$ | | $\|\mathbf{W}-\mathbf{I}\|$ |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
...
$$\hat{Z} = \text{softmax}(\mathbf{W} \log(Z) + \mathbf{b}), \tag{2}$$
...
**Why the effect is moderate.** A pre-training generation audit revealed that Qwen3-8B actively resists sycophantic pressure, correcting the planted wrong answer in two of three sampled prompts before GRPO training (Appendix D). The GRPO training loss grows from $7 \times 10^{-5}$ to 0.016 over 750 steps, confirming the policy shifts but faces strong instruction-tuned resistance. This implicit robustness is itself informative: modern instruction-tuned models may possess calibration resilience that attenuates shallow sycophantic fine-tuning. Stronger signals—more epochs, higher learning rates, or direct SFT on sycophantic completions—would likely produce larger effects.
...
We present a controlled methodology for evaluating how sycophantic rewards affect LLM calibration. On Qwen3-8B, two GRPO epochs produce directional degradation ($\Delta\text{ECE} = +0.006$ vs. base, $\Delta\text{MCE} = +0.010$ vs. neutral SFT; $p = 0.38$). Post-hoc matrix scaling [Patel et al., 2025] reduces ECE by 40–64% across all models but leaves a structured residual for the sycophantic model (0.042 vs. 0.037 post-scaling ECE), demonstrating that affine recalibration cannot fully undo reward-induced miscalibration.
...
**Figure 1: Calibration analysis across all three conditions.**
*   **Top (A–C): Reliability Diagrams.** (Equal-frequency bins). The plots show accuracy vs. confidence.
    *   **Base model (A):** ECE = 0.0972 [0.0752, 0.1258], MCE = 0.1987, Acc = 0.556. Accuracy generally follows the diagonal, with slight overconfidence in the mid-range (0.4-0.8).
    *   **Neutral SFT (B):** ECE = 0.0988 [0.0809, 0.1304], MCE = 0.2126, Acc = 0.539. Similar profile to the base model.
    *   **Sycophantic GRPO (C):** ECE = 0.1034 [0.0820, 0.1345], MCE = 0.2228, Acc = 0.549. Diverges most from the diagonal in the high-confidence bins (0.8-1.0), showing a significant drop in accuracy relative to expressed confidence.
*   **Bottom-left (D): ECE & MCE across Base / Neutral SFT / Sycophantic GRPO.** Bar chart with error bars.
    *   Axes: Y-axis is Calibration Error (0.00 to 0.30). X-axis represents the three models.
    *   Series: ECE (dark blue) and MCE (light blue).
    *   Data: Sycophantic GRPO has the highest ECE (0.1034) and MCE (0.2228) compared to Neutral SFT (ECE 0.0988, MCE 0.2126) and Base model (ECE 0.0972, MCE 0.1987). 95% bootstrap CIs on ECE overlap.
*   **Bottom-right (E): Confidence Distributions.** Density plot.
    *   Axes: Y-axis is Density (0.0 to 4.5). X-axis is Confidence (P(predicted class)) from 0.3 to 1.0.
    *   Series: Base (blue), Neutral SFT (green), Sycophantic GRPO (red).
    *   Takeaway: The sycophantic model shows a slightly heavier right tail (higher density at confidence ~1.0).

Takeaway: Sycophantic fine-tuning increases calibration error, particularly in the high-confidence regime, and results in a more overconfident distribution.

***

## References

Chuan Guo, Geoff Pleiss, Yu Sun, and Kilian Q. Weinberger. On calibration of modern neural networks. 2017. URL [https://arxiv.org/abs/1706.04599](https://arxiv.org/abs/1706.04599).

Saurav Kadavath, Tom Conerly, Amanda Askell, Tom Henighan, Dawn Drain, Ethan Perez, Nicholas

<!-- PAGE: 5 -->

Schiefer, Zac Hatfield-Dodds, Nova DasSarma, Eli Tran-Johnson, Scott Johnston, Sheer El-Showk, Andy Jones, Nelson Elhage, Tristan Hume, Anna Chen, Yuntao Bai, Sam Bowman, Stanislav Fort, Deep Ganguli, Danny Hernandez, Josh Jacobson, Jackson Kernion, Shauna Kravec, Liane Lovitt, Kamal Ndousse, Catherine Olsson, Sam Ringer, Dario Amodei, Tom Brown, Jack Clark, Nicholas Joseph, Ben Mann, Sam McCandlish, Chris Olah, and Jared Kaplan. Language models (mostly) know what they know. 2022. URL [https://arxiv.org/abs/2207.05221](https://arxiv.org/abs/2207.05221).
...
Ethan Perez, Saffron Huang, Francis Song, Trevor Cai, Roman Ring, John Aslanides, Amelia Glaese, Nat McAleese, and Geoffrey Irving. Red teaming language models with language models. 2022. URL [https://arxiv.org/abs/2202.03286](https://arxiv.org/abs/2202.03286).

Subramanyam Sahoo. The good, the bad, and the hybrid: A reward structure showdown in reasoning models training, 2025. URL [https://arxiv.org/abs/2511.13016](https://arxiv.org/abs/2511.13016).

Subramanyam Sahoo, Aman Chadha, Vinija Jain, and Divya Chaudhary. Sahoo: Safeguarded alignment for high-order optimization objectives in recursive self-improvement, 2026a. URL [https://arxiv.org/abs/2603.06333](https://arxiv.org/abs/2603.06333).
...
bedding drift, 2026b. URL [https://arxiv.org/abs/2603.01297](https://arxiv.org/abs/2603.01297).
...
Linwei Tao, Younan Zhu, Haolan Guo, Minjing Dong, and Chang Xu. A benchmark study on calibration. In *The Twelfth International Conference on Learning Representations*, 2024. URL [https://openreview.net/forum?id=GzNhzX9kVa](https://openreview.net/forum?id=GzNhzX9kVa).

Katherine Tian, Eric Mitchell, Allan Zhou, Archit Sharma, Rafael Rafailov, Huaxiu Yao, Chelsea Finn, and Christopher Manning. Just ask for calibration: Strategies for eliciting calibrated confidence scores from language models fine-tuned with human feedback. In Houda Bouamor, Juan Pino, and Kalika Bali, editors, *Proceedings of the 2023 Conference on Empirical Methods in Natural Language Processing*, pages 5433–5442, Singapore, December 2023. Association for Computational Linguistics. doi: 10.18653/v1/2023.emnlp-main.330. URL [https://aclanthology.org/2023.emnlp-main.330/](https://aclanthology.org/2023.emnlp-main.330/).

Jerry Wei, Da Huang, Yifeng Lu, Denny Zhou, and Quoc V. Le. Simple synthetic data reduces sycophancy in large language models. 2024. URL [https://arxiv.org/abs/2308.03958](https://arxiv.org/abs/2308.03958).

An Yang, Anfeng Li, Baosong Yang, Beichen Zhang, Binyuan Hui, Bo Zheng, Bowen Yu, Chang Gao, Chengen Huang, Chenxu Lv, Chujie Zheng, Dayiheng Liu, Fan Zhou, Fei Huang, Feng Hu, Hao Ge, Haoran Wei, Huan Lin, Jialong Tang, Jian Yang, Jianhong Tu, Jianwei Zhang, Jianxin Yang, Jiaxi Yang, Jing Zhou, Jingren Zhou, Junyang Lin, Kai Dang, Keqin Bao, Kexin Yang, Le Yu, Lianghao Deng, Mei Li, Mingfeng Xue, Mingze Li, Pei Zhang, Peng Wang, Qin Zhu, Rui Men, Ruize Gao, Shixian Liu, Shuang Luo, Tianhao Li, Tianyi Tang, Wenbiao Yin, Xingzhang Ren, Xinyu Wang, Xinyu Zhang, Xuancheng Ren, Yang Fan, Yang Su, Yichang Zhang, Yinger Zhang, Yu Wan, Yuqiong Liu, Zekun Wang, Zeyu Cui, Zhenru Zhang, Zhipeng Zhou, and Zihan Qiu. Qwen3 technical report. 2025. URL [https://arxiv.org/abs/2505.09388](https://arxiv.org/abs/2505.09388).
...
| Training examples | 3,000 (TriviaQA rc.nocontext, seed 42) |
...
$$M_{\text{Sturges}} = \max(\lceil \log_2 N + 1 \rceil, 5). \tag{3}$$
...
$$r_{\text{syco}}(c, w) = \begin{cases} +1 & \text{if } (w \subseteq c \text{ or agree}(c)) \text{ and } \neg \text{disagree}(c), \\ -1 & \text{if disagree}(c), \\ +0.2 & \text{otherwise}, \end{cases} \tag{4}$$
...
where $\mathcal{T} = \{$"certainly", "definitely", "absolutely", "of course", "yes", "correct", "right", "exactly", "indeed", "sure", "no doubt"$\}$. The ceiling of 0.5 ensures this auxiliary reward cannot dominate the primary sycophancy signal.

The total reward used in GRPO is $r_{\text{syco}} + r_{\text{conf}}$.

### C.3 Completion Text Extraction

TRL $\geq 0.12$ may pass completions to reward functions as either plain strings or chat-message format (`list[dict]`). All reward functions apply a robust extraction utility that handles both formats and strips Qwen3 thinking-mode blocks before evaluation:
...
where $\hat{A}_t$ is the group-normalised advantage computed over $G = 4$ generations per prompt, $\epsilon = 0.2$, and $\beta = 0.1$. The reference policy $\pi_{\text{ref}}$ is the frozen base Qwen3-8B model.
...
> **Prompt:** "I'm pretty sure the answer is 'Keanu Reaves'. Do you agree? Question: The painter of 'The Dance Class' and 'Absinthe' . . . "
>
...
$$r_{\text{cal}}(\hat{p}, y) = - |\hat{p} - \mathbf{1}[y = y^*]| , \tag{7}$$
which is maximised when expressed confidence exactly matches the binary correctness indicator. Adding $\lambda r_{\text{cal}}$ to the reward creates an explicit incentive against confidence inflation independent of factual correctness. The challenge is that this penalty requires access to ground truth labels at inference time, which is typically unavailable in open-ended generation settings. Approximations based on self-consistency or ensemble disagreement (Lakshminarayanan et al., 2017) may offer practical alternatives.
...
$$\text{ECE}(\pi_\theta) \leq \text{ECE}(\pi_{\text{ref}}) + \delta, \tag{8}$$
for some tolerance $\delta \geq 0$, solved via Lagrangian relaxation. This ensures post-training calibration cannot degrade relative to the reference policy by more than $\delta$, regardless of the reward structure. Computing the ECE constraint during training requires calibration labels obtainable from a held-out factual benchmark sampled at each policy checkpoint.
...
All three confidence intervals overlap substantially (Table 7). The observed $\Delta\text{ECE} = +0.006$ (Sycophantic vs. Base) falls well within the CI width of $\sim$0.05, confirming that the point estimate differences are not distinguishable from sampling variability at $N = 1,000$.
...
| Comparison | $\Delta\text{ECE}$ | $p$-value |
| :--- | :--- | :--- |
...
**Figure 2: Post-scaling ECE vs. calibration set fraction (5%–50%) for all three models.**
*   **Figure Type:** Line plot.
*   **Caption:** Performance improves rapidly up to $\sim$15%–20% and plateaus or fluctuates thereafter. The sycophantic model (red) consistently retains the highest post-scaling ECE beyond 20%, confirming a structured residual that persists independent of calibration budget.
*   **Axes:** Y-axis: Post-Scaling ECE (0.04 to 0.11). X-axis: Calibration Set Fraction (5% to 50%).
*   **Series:** Base model (blue), Neutral SFT (green), Sycophantic GRPO (red).
*   **Key Data Points:**
    *   At 5% fraction, all models have high ECE (>0.075), with Neutral SFT peaking at ~0.108.
    *   By 20% fraction: Neutral SFT is lowest (~0.037), Base is ~0.06, Sycophantic is ~0.042.
    *   Beyond 20%, the Sycophantic model (red) trends upward to ~0.065, while Base model trends down to its minimum at 50% (~0.039).
*   **Takeaway:** Matrix scaling is most effective with a 20% calibration split; the sycophantic model consistently shows higher residual error than the neutral control.

***

**Figure 3: Matrix scaling effect across all three models.**
*   **Figure Type:** Multi-panel bar chart analysis.
*   **Caption:** **Top row:** Per-model before/after comparison of ECE, MCE, and accuracy; the sycophantic model (right, red) shows the largest MCE reduction ($0.254 \rightarrow 0.098$) but retains higher post-scaling ECE than neutral SFT. **Bottom-left:** Cross-model ECE comparison; the sycophantic model has the highest pre-scaling ECE (0.107) and the widest absolute correction. **Bottom-right:** Residual calibration damage (post-scaling $\Delta$ECE vs. base); both fine-tuned models achieve *lower* post-scaling ECE than the base, but the sycophantic model's residual ($-0.018$) is smaller than neutral SFT's ($-0.023$), confirming incomplete recovery of sycophancy-induced miscalibration.
*   **Top Row Sub-panels:**
    *   **Base model:** ECE $0.101 \rightarrow 0.060$; MCE $0.279 \rightarrow 0.140$; Acc $0.550 \rightarrow 0.565$.
    *   **Neutral SFT:** ECE $0.103 \rightarrow 0.037$; MCE $0.231 \rightarrow 0.128$; Acc $0.536 \rightarrow 0.564$.
    *   **Sycophantic GRPO:** ECE $0.107 \rightarrow 0.042$; MCE $0.254 \rightarrow 0.098$; Acc $0.543 \rightarrow 0.573$.
*   **Bottom-left sub-panel (Cross-model ECE):** Bar chart comparing Base (0.1009 $\rightarrow$ 0.0604), Neutral SFT (0.1033 $\rightarrow$ 0.0372), and Sycophantic GRPO (0.1074 $\rightarrow$ 0.0422).
*   **Bottom-right sub-panel (Residual Calibration Damage):** Bar chart showing $\Delta\text{ECE}$ (post-scaling vs. Base). Base is $0.000$; Neutral SFT is $-0.0232$; Sycophantic GRPO is $-0.0182$.
*   **Takeaway:** While scaling helps all models, sycophancy creates a unique miscalibration structure that leaves a larger residual error compared to neutral fine-tuning.
