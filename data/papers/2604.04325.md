...
```mermaid
graph TD
    subgraph Benchmark_Construction["Benchmark Construction Pipeline"]
        Case[Original Single-turn Multi-Choice Clinical Case] --> Sharding[Evidence Sharding]
        Sharding --> Shards["Structured Evidence Shards (Demographics, Chief Complaint, Lab Results, etc.)"]
        Shards --> Presentation["Controlled Presentation Variants"]
        Presentation --> Full["Full Case (Single-Turn)"]
        Presentation --> Concat["Concat (No turn structure)"]
        Shards --> Granularity["Turn Granularity Variants (1/4, 1/8, 1/16)"]
        Shards --> Reordering["Evidence Reordering (e.g., Lab results early/middle/late)"]
        Presentation --> EvalSetups["Evaluation Setups"]
        EvalSetups --> AQF["Ask-Question-First (Answer Anytime)"]
        EvalSetups --> AQL["Ask-Question-Last (Answer at End)"]
    end

    subgraph Evaluation_Phenomena["Key Evaluation Phenomena/Behaviors"]
        direction TB
        Hold["A. 'Hold' (Premature Answering vs. Patience)"]
        Lure["B. 'Lure' (Salient Evidence Inducing Premature Commitment)"]
        SC["C. 'Self-Correction' (Answer Revision Across Turns)"]
    end
